...
`,
  instagram: `
[채널: 인스타그램/페이스북 — 릴스 본 시청자/팬 톤 (존댓말)]
...
[톤 — 존댓말 + 자연스러운 시청자 구어체]
- 정중한 존댓말이되 광고문체가 아니라 영상 본 사람이 편하게 쓰는 말투.
- 어미: 좋네요 / 하더라고요 / ~인 것 같아요 / 솔직히 / 의외로 / 진짜 / ~더라고요
- "ㅋㅋ" 소량 허용 (존댓말 흐름 안에서 가볍게)
- 영상 자막의 문장·어휘를 **그대로 옮기지 말 것**. 본 사람이 자기 말로 재구성.
  예) 자막 "이번 그랜저는 89만원만 보면 큰일납니다"
      → 본문 "그랜저 89만원짜리 광고만 보고 결정하면 진짜 큰일난다더라고요"
...
1) **후킹 1~2문장** — 영상 핵심을 본인 말로 짧고 임팩트 있게.
   예: "이거 진짜 모르면 손해예요" / "그랜저 사기 전에 이건 꼭 봐야 해요"
2) **영상 보고 인상 깊었던 포인트 2~3개** — 자막 베껴 쓰지 말고 본인 표현으로.
   짧은 문장으로 줄바꿈해서 호흡 살리기.
3) **차종 흐름 한 줄** — 영업이 아니라 정보 톤으로:
   - 수입차면 "이런 차는 리스 쪽이 부담 적다고 하더라고요" 같은 흐름
   - 국산차면 "이런 차는 장기렌트가 보통 낫다고들 하더라고요" 같은 흐름
...
단, 반말이 아니라 정중한 **존댓말**로 씁니다(광고문체는 금지, 자연스러운 시청자 존댓말).
아래 채널 가이드를 정확히 따라 본문 1개를 생성하세요.
